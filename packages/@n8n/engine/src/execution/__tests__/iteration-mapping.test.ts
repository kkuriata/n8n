...
describe('classifyEdge', () => {
	...
	it('classifies an edge from one loop into the next as exit', () => {
		// two simple loops chained: B1's done slot feeds B2
...
describe('targetKey', () => {
	it('keeps the iteration inside a pass', () => {
	...
	it('feeds iteration 0 through the entry edge', () => {
...
describe('sourceRow', () => {
	...
	it('reads the same pass inside the loop', () => {
		expect(sourceRow(edge('B', 'x'), 'intra', { nodeId: 'x', iteration: 2 })).toEqual(row('B', 2));
	});

	it('reads the previous pass across the return edge', () => {
	...
	it('reads the entry edge at iteration 0 only, later passes coming from the return edge', () => {
	...
	/**
	 * The distinction the caller depends on: an edge that connects nothing here is
	 * ignored, while an edge waiting for a terminal row holds the decision open.
	 * Collapsing them either deadlocks later passes or settles the done side of a
	 * loop still running.
	 */
	it('tells an edge that does not apply apart from one waiting for the loop to end', () => {
		// the return edge feeds passes 1 upwards, so at iteration 0 it connects nothing
		expect(sourceRow(edge('x', 'B'), 'back', { nodeId: 'B', iteration: 0 })).toEqual({
			kind: 'none',
		});

		// the exit edge applies, but the terminal row it reads does not exist yet (L2)
